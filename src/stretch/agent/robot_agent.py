...
class RobotAgent:
    ...
    def place_object(self, object_goal: Optional[str] = None, **kwargs) -> bool:
        ...
        if self.grasp_client is None:
            logger.warning("Tried to place without providing a grasp client.")
            return False
    ...
    def grasp_object(self, object_goal: Optional[str] = None, **kwargs) -> bool:
        ...
        if not self.robot.in_manipulation_mode():
            self.robot.switch_to_manipulation_mode()
        if self.grasp_client is None:
            logger.warning("Tried to grasp without providing a grasp client.")
            return False
    ...
    def start(
        ...
        if verbose:
            print("Segway Client connection to robot started.")
