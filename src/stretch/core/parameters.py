...
class Parameters(object):
    ...
    def get_task_goals(self) -> Tuple[str, str]:
        """Helper for extracting task information: returns the two different task goals for a very simple OVMM-style (pick, place) task."""
        if "object_to_find" in self.data:
            object_to_find = self.data["object_to_find"]
            if len(object_to_find) == 0:
                object_to_find = None
        else:
            object_to_find = None
        if "location_to_place" in self.data:
            location_to_place = self.data["location_to_place"]
    ...
    def from_yaml(yaml_file):
        """Load parameters from a YAML file (compatibility method)"""
        return Parameters.load(yaml_file)
