...
# ----------------------------------------------------
"""
Constants for perception modules
"""

# COCO classes used for object detection
COCO_CLASSES = [
    "chair", "couch", "potted plant", "bed", "toilet", "tv", 
    "dining table", "oven", "sink", "refrigerator", "book", 
    "clock", "vase", "cup", "bottle", "no-category"
]

# Custom classes for Segway-specific detection
CUSTOM_CLASSES = [
    "person", "robot", "obstacle", "door", "elevator", "stairs"
]
coco_categories = {
    "chair": 0,
    "couch": 1,
...
# )
languagenav_2categories_color_palette = [255, 255, 255] + list(d3_40_colors_rgb[1:].flatten())
...
class LanguageNavCategories(SemanticCategoryMapping):
    ...
    def reset_instance_id_to_category_id(self, env):
        ...
        for obj in env.sim.semantic_annotations().objects:
            ...
            if category is None:
                self._instance_id_to_category_id.append(self.all_hm3d_categories.index("unknown"))
            else:
                self._instance_id_to_category_id.append(self.all_hm3d_categories.index(category))
...
hssd_28categories_padded = ["."] + [hssd_28categories_indexes[i] for i in range(1, 29)] + ["other"]

hssd_28categories_legend_path = str(Path(__file__).resolve().parent / "hssd_28_cat_legend.png")

hssd_28categories_color_palette = [255, 255, 255] + list(d3_40_colors_rgb[1:34].flatten())
        ...
        self._instance_id_to_category_id = np.ndarray(
            [
                long_tail_indoor_categories.index(
                    hm3d_to_longtail_indoor.get(obj.category.name().lower().strip(), "other")
                )
                for obj in env.sim.semantic_annotations().objects
            ]
        )
